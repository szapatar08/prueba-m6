...
public class BookingCancelledEventHandlerTests
{
    ...
    public async Task HandleAsync_ShouldCreateNotification()
    {
        ...
        _templateRendererMock.Setup(t => t.Render(TemplateTypes.BookingCancelled, It.IsAny<object>()))
    ...
    public async Task HandleAsync_ShouldUseTemplateRendererForEmail()
    {
        ...
        SetupGuestAndBooking(guestId, bookingId, propertyId);
        _templateRendererMock.Setup(t => t.Render(TemplateTypes.BookingCancelled, It.IsAny<object>()))
            .Returns(("Booking Cancelled - Beach House", "<html>cancelled</html>"));

        // Act
        await _handler.HandleAsync(bookingId, guestId);

        // Assert — template renderer used, NOT hardcoded email
        _templateRendererMock.Verify(t => t.Render(TemplateTypes.BookingCancelled, It.IsAny<object>()), Times.Once);
        _emailServiceMock.Verify(e => e.SendEmailAsync(
            "real.guest@domain.com",
            "Booking Cancelled - Beach House",
            "<html>cancelled</html>",
    ...
    public async Task HandleAsync_ShouldResolveGuestEmailFromUserEntity()
    {
        ...
        _emailServiceMock.Verify(e => e.SendEmailAsync(
            ...
            It.IsAny<string>(),
            CancellationToken.None), Times.Once);
        // Verify NO call to hardcoded address
        _emailServiceMock.Verify(e => e.SendEmailAsync(
            "guest@example.com",
            It.IsAny<string>(),
            It.IsAny<string>(),
